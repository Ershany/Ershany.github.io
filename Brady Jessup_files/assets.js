function init() {
    ...
    }, mapElement = document.getElementById("map"), map = new google.maps.Map(mapElement, mapOptions), image = "img/map-marker.png", myLatLng = new google.maps.LatLng(40.67, -73.94);
...
if (function(global, factory) {
    ...
    $.ui.resizable, $.widget("ui.selectable", $.ui.mouse, {
        ...
        _mouseStart: function(event) {
            ...
            this._trigger("start", event), $(options.appendTo).append(this.helper), this.helper.css({
                left: event.pageX,
                top: event.pageY,
                width: 0,
                height: 0
        ...
        _eventHandler: function(event) {
            ...
            this.active = clickedIsActive ? $() : clicked, this._toggle(eventData), active.removeClass("ui-accordion-header-active ui-state-active"), 
    ...
    var dataSpace = ($.datepicker, $.widget("ui.dialog", {
        version: "1.11.4",
        options: {
            appendTo: "body",
            autoOpen: !0,
        ...
        _setOption: function(key, value) {
            "max" === key && (value = Math.max(this.min, value)), "disabled" === key && this.element.toggleClass("ui-state-disabled", !!value).attr("aria-disabled", value), 
            this._super(key, value);
        },
        ...
        _setOption: function(key, value) {
            ...
            key) {
              case "orientation":
                ...
                this._refreshValue(), this.handles.css("horizontal" === value ? "bottom" : "left", "");
                break;
        ...
        _draw: function() {
            ...
            this.element.attr("role", "spinbutton"), this.buttons = uiSpinner.find(".ui-spinner-button").attr("tabIndex", -1).button().removeClass("ui-corner-all"), 
        ...
        _create: function() {
            ...
            }))).sort()), this.options.active !== !1 && this.anchors.length ? this.active = this._findActive(options.active) : this.active = $(), 
        ...
        _open: function(event, target, content) {
            ...
            if (content) {
                ...
                content.clone ? (a11yContent = content.clone(), a11yContent.removeAttr("id").find("[id]").removeAttr("id")) : a11yContent = content, 
    ...
    $.effects, $.effects.effect.blind = function(o, done) {
        ...
        for ((show || hide) && props.push("opacity"), $.effects.save(el, props), el.show(), 
        $.effects.createWrapper(el), distance || (distance = el["top" === ref ? "outerHeight" : "outerWidth"]() / 3), 
        show && (downAnim = {
            opacity: 1
        ...
        "hide" === mode && (options.from.opacity = 1, options.to.opacity = 0)), el.effect(options);
        ...
        for ($.effects.save(el, props), el.show(), $.effects.createWrapper(el), animation[ref] = (positiveMotion ? "-=" : "+=") + distance, 
